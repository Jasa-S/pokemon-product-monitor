...
GitHub Actions checks both stores' newest backend products and selected Pokémon Store restocks every five minutes. A separate daily job refreshes the complete public Pokémon Store catalog—including sold-out products—and a broad deduplicated snapshot from every public Naver Brand Store leaf category. Both update the SQLite state and `docs/status.json`; GitHub Pages serves the dashboard from `docs/`. Scheduled Actions can be delayed during busy periods, so five minutes is the target cadence rather than a real-time guarantee.
